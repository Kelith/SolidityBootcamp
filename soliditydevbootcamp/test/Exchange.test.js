    ...
    describe('fillOrder()', () => {
        describe('Check balances after filling user1 buy Tokens order', () => {
          beforeEach(async () => {
            ...
            await exchange.makeOrder(token.address, tokens(10), ETHER_ADDRESS, tokens(1), {from: user1})
            // user2 gets tokens
          ...
          it('user2 ether balance on exchange should eq. 1', async () => {
            await (await exchange.balanceOf(ETHER_ADDRESS, user2)).toString().should.eq(tokens(1).toString())
          })
        })
        ...
        describe('Check balances after filling user1 buy Ether order', () => {
          beforeEach(async () => {
            ...
            await exchange.makeOrder(ETHER_ADDRESS, tokens(1), token.address, tokens(10), {from: user1})
            // user2 deposit 1 ETHER + fee cost (.1 ETH) to the exchange
            await exchange.depositEther({from: user2, value: tokens(1.1)})
            // user2 fills the order
            await exchange.fillOrder('1', {from: user2})
          })
          ...
          it('user1 Ether balance on exchange should eq. 1', async () => {
            await (await exchange.balanceOf(ETHER_ADDRESS, user1)).toString().should.eq(tokens(1).toString())
          })
